...
namespace gzh
{
	...
	void pool::release_connect(MYSQL* connect)
	{
		...
		std::cout <<"thread_id:" << std::this_thread::get_id() << "  回收了数据库连接"  << connect_pool.size() << std::endl;
	}
	...
	MYSQL* pool::get_connection(bool& is_last_connect)
	{
		...
		if (connect_pool.size() > 1)
		{
			...
			std::cout << "thread_id:" << std::this_thread::get_id() << "  取用了一个数据库连接  " << "剩下 " << connect_pool.size() << std::endl;
			return mysql;
		}
		else if (connect_pool.size() == 1)
		{
			...
			std::cout << "thread_id:" << std::this_thread::get_id() << "   取用了最后一个数据库连接 " << std::endl;
			std::cout << "thread_id:" << std::this_thread::get_id() << "   ************进入单例模式************" << std::endl;
			return mysql;
		}
		else 
		{
